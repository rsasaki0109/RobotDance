...
- [x] **G1 morphology を実 URDF 実寸に修正**（real-data validation）: 旧手書きプロキシ（nominal 1.12m・
      bone 相対誤差 ~26%）を公式 g1_23dof URDF 由来の実寸へ更新（nominal 1.291m・誤差 ~0%）。実 URDF で
      actuator IK 収束（mean err 0.062m, limit 違反 0）も確認。`tests/test_real_g1_urdf.py` で回帰検証
