...
- [x] RD-Embodiment: G1 config（v0 簡略 kinematic プロキシ / generic limits）。実 URDF・SDK2 写像・H1 は今後
- [x] retargeting engine v0: direction-preserving FK + morphology normalization + ground clamp
- [x] `.rdmotion` artifact + `rd-motion.schema.json` 確定（v0）
- [x] viewer: human ↔ G1 side-by-side GIF（`view-pair` / `demo-g1`）
