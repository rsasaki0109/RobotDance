...
- [x] **joint-space safety guard v0**（`demo-joint-safety`, `clamp_joint_trajectory`）: 関節角列を
      実機コマンド直前に **位置 limit・速度・加速度**へクランプ（`sim_certificate` の先の最終 gate,
      純 numpy・CI 検証）。位置/速度は厳密 bound・加速度は best-effort。トルク/電流 limit は今後
...
- [x] **safety guard v0** ✅（Cartesian + **joint/velocity/accel guard**, fall detector, E-stop, sim
      certificate check）— torque/電流 guard は実機モデルが入る時に追加
