...
| `fourier_n1` | mujoco_menagerie `fourier_n1`（Fourier N1 由来） | Apache-2.0 | 39.727 kg | 1.145 m | mujoco | 200 / 8 |
...
- **Fourier N1**: MuJoCo Menagerie model（Fourier Intelligence N1 由来）, Apache-2.0
  （[google-deepmind/mujoco_menagerie](https://github.com/google-deepmind/mujoco_menagerie)）。
  v0.125 で fourier_n1（n1.xml）由来の実寸/質量/慣性から追加（nominal 1.15m / 39.7kg）。トルク・速度は
  menagerie MJCF に未収載のため feasibility の torque/velocity 軸は generic fallback。
